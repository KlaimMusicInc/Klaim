...
# ============== Usuarios a cargar ==============
USERS = [
    # ("ADMINISTRADOR",     "1121947382Dd.", "David Cubillos",                                         "Administrador", None),
    # ("AlejandraCardona",  "ACKlaim2025.",  "Maria Alejandra Cardona",                                 "Superstaff",    None),
    # ("DavidCubillos",     "97072201144Dd.",  "David Cubillos Pinto",                                   "Superstaff",    None),
...
def resolve_cliente_id(
    cur, cliente_id: Optional[int], nombre_cliente_hint: Optional[str]
) -> Optional[int]:
    if cliente_id is not None:
        cur.execute(
            "SELECT id_cliente FROM clientes WHERE id_cliente=%s", (int(cliente_id),)
        )
        row = cur.fetchone()
        return int(row[0]) if row else None
    if nombre_cliente_hint:
        cur.execute(
            "SELECT id_cliente FROM clientes WHERE nombre_cliente=%s",
            (nombre_cliente_hint,),
        )
        row = cur.fetchone()
        return int(row[0]) if row else None
...
def upsert_user(
    ...
    if role not in VALID_ROLES:
        raise ValueError(
            f"Role inválido: {role!r}. Debe ser uno de {sorted(VALID_ROLES)}"
        )
    ...
    if role == "Cliente":
        ...
        if not resolved_id:
            cur.execute(
                "SELECT id_cliente, nombre_cliente FROM clientes ORDER BY id_cliente"
            )
            catalog = cur.fetchall()
            listado = (
                ", ".join(f"{cid}:{nom}" for cid, nom in catalog) or "(sin registros)"
            )
            print(
                f"⚠ No se pudo vincular cliente para {username}: "
...
def main():
    ap = argparse.ArgumentParser(
        description="Seed de usuarios (auth_user) compatible con Django."
    )
    ap.add_argument(
        "--env", choices=["prod", "dev"], default=None, help="Perfil predefinido de BD."
    )
    ...
    ap.add_argument(
        "--reset",
        action="store_true",
        help="Trunca tablas de usuarios antes de sembrar (peligroso).",
    )
    ap.add_argument(
        "--no-interactive",
        action="store_true",
    ...
    if args.no_interactive:
        cfg = (
            DEFAULT_PROFILES[args.env].copy()
            if args.env
            else DEFAULT_PROFILES["prod"].copy()
        )
        if args.db_name:
            cfg["database"] = args.db_name
        if args.db_user:
            cfg["user"] = args.db_user
        if args.db_pass:
            cfg["password"] = args.db_pass
        if args.db_host:
            cfg["host"] = args.db_host
        if args.db_port:
            cfg["port"] = args.db_port
        print(
            f"Conectando a {cfg['user']}@{cfg['host']}:{cfg['port']} DB={cfg['database']} | ALGO={args.algo}"
        )
    else:
    ...
    try:
        ...
        if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
            print("❌ Credenciales de DB inválidas")
