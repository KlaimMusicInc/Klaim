...
def find_sheet_and_header(wb, required: List[str]) -> Tuple[Optional[str], Optional[int]]:
...
# ============== Builders =================
def build_ms_from_row(row: List[Any], pos: Dict[str, int], id_file: int) -> MicroSyncStatement:
...
def build_share_from_row(row: List[Any], pos: Dict[str, int], id_file: int) -> MicroSyncMarketShare:
    get = lambda col: row[pos[col]] if pos[col] < len(row) else None
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("xlsx_path", type=str, help="Ruta al archivo XLSX (MicroSync)")
        ...
        parser.add_argument("--q", dest="q", type=int, choices=[1, 2, 3, 4],
        ...
        parser.add_argument("--nombre-archivo", dest="nombre_archivo", type=str, default=None)
        ...
        parser.add_argument("--detalle-sheet", dest="detalle_sheet", type=str, help="Nombre hoja DETALLE")
        parser.add_argument("--detalle-header-row", dest="detalle_header_row", type=int, help="Fila encabezado DETALLE (1-based)")
        parser.add_argument("--cuota-sheet", dest="cuota_sheet", type=str, help="Nombre hoja CUOTA")
        parser.add_argument("--cuota-header-row", dest="cuota_header_row", type=int, help="Fila encabezado CUOTA (1-based)")
    ...
    def handle(self, *args, **opts):
        ...
        if not annual and not q_opt:
            raise CommandError("Debes indicar --q 1..4 o usar --annual para archivo anual.")
        ...
        if detalle_sheet and detalle_header_row:
            ...
            if not sheet_auto:
                raise CommandError("No se encontró hoja con columnas de DETALLE requeridas.")
        ...
        det_header_cells = next(ws_det.iter_rows(min_row=det_header_idx + 1, max_row=det_header_idx + 1, values_only=True))
        ...
        if ws_cuota is not None and cuota_header_idx is not None:
            cuota_header_cells = next(ws_cuota.iter_rows(min_row=cuota_header_idx + 1, max_row=cuota_header_idx + 1, values_only=True))
        ...
        if dry_run:
            ...
            if ws_cuota and cuota_pos:
                ...
                for row in ws_cuota.iter_rows(min_row=cuota_header_idx + 2, values_only=True):
                    if row is None:
                        continue
                    total_cuota += 1
        ...
        if ws_cuota and cuota_pos:
            ...
            for row in ws_cuota.iter_rows(min_row=cuota_header_idx + 2, values_only=True):
                if row is None:
                    continue
                batch_share.append(build_share_from_row(list(row), cuota_pos, id_file=sf.id_file))
                if len(batch_share) >= batch_size:
                    with transaction.atomic():
                        MicroSyncMarketShare.objects.bulk_create(batch_share, batch_size=batch_size)
