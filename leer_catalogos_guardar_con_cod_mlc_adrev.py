...
def cliente_ya_insertado(connection, nombre_cliente):
    query = text(
        "SELECT id_cliente FROM Clientes WHERE nombre_cliente = :nombre_cliente"
    )
...
def insert_client(connection, nombre_cliente, tipo_cliente):
    ...
    query = text(
        "INSERT INTO Clientes (nombre_cliente, tipo_cliente) VALUES (:nombre_cliente, :tipo_cliente)"
    )
    connection.execute(
        query, {"nombre_cliente": nombre_cliente, "tipo_cliente": tipo_cliente}
    )
...
def insert_catalog(connection, id_cliente, fecha_recibido, estado="Activo"):
    query = text(
        "INSERT INTO Catalogos (id_cliente, fecha_recibido, estado) VALUES (:id_cliente, :fecha_recibido, :estado)"
    )
    connection.execute(
        query,
        {"id_cliente": id_cliente, "fecha_recibido": fecha_recibido, "estado": estado},
    )
    return connection.execute(text("SELECT LAST_INSERT_ID()")).fetchone()[0]
...
def artista_unico_ya_insertado(connection, nombre_artista):
    query = text(
        "SELECT id_artista_unico FROM artistas_unicos WHERE nombre_artista = :nombre_artista"
    )
...
def insert_artista_unico(connection, nombre_artista):
    ...
    query = text(
        "INSERT INTO artistas_unicos (nombre_artista) VALUES (:nombre_artista)"
    )
...
def insert_artistas(connection, nombre_artista, obra_id):
    ...
    query = text(
        "INSERT INTO artistas (nombre_artista, obra_id, id_artista_unico) VALUES (:nombre_artista, :obra_id, :id_artista_unico)"
    )
    connection.execute(
        query,
        {
...
def autor_ya_insertado(connection, nombre_autor, codigo_ipi, tipo_autor):
    query = text(
        "SELECT id_autor FROM AutoresUnicos WHERE nombre_autor = :nombre_autor AND codigo_ipi = :codigo_ipi AND tipo_autor = :tipo_autor"
    )
...
def insert_autores_unicos(connection, nombre_autor, codigo_ipi, tipo_autor):
    autor_id_existente = autor_ya_insertado(
        connection, nombre_autor, codigo_ipi, tipo_autor
    )
    if autor_id_existente:
        return autor_id_existente
    query = text(
        "INSERT INTO AutoresUnicos (nombre_autor, codigo_ipi, tipo_autor) VALUES (:nombre_autor, :codigo_ipi, :tipo_autor)"
    )
    connection.execute(
        query,
        {
...
# Insertar datos en la tabla subidas_plataforma
def insert_subidas_plataforma(connection, obra_id, codigo_mlc, codigo_adrev):
    query = text(
        """
        INSERT INTO subidas_plataforma (obra_id, codigo_MLC, codigo_ADREV, fecha_subida)
        VALUES (:obra_id, :codigo_mlc, :codigo_adrev, :fecha_subida)
    """
    )
    connection.execute(
        query,
        {
            "obra_id": obra_id,
            "codigo_mlc": codigo_mlc,
            "codigo_adrev": codigo_adrev,
            "fecha_subida": date.today(),
        },
    )


# Modificar la función process_and_load_catalog para manejar las columnas adicionales
def process_and_load_catalog(
    file_path, connection, nombre_cliente, tipo_cliente, fecha_recibido
):
    ...
    for index, row in catalogo_df.iterrows():
        ...
        if pd.isna(titulo) or pd.isna(codigo_sgs):
            print(
                f"Fila {index + 1} contiene valores nulos: Título: {titulo}, Código SGS: {codigo_sgs}"
            )
        ...
        if (titulo != obra_actual) or (codigo_sgs != codigo_sgs):
            ...
            obra_id_actual = insert_obras(
                connection, titulo, codigo_sgs, codigo_iswc, catalogo_id
            )
        ...
        numero_ip_autor = (
            row["Número IP Autor"] if pd.notna(row["Número IP Autor"]) else None
        )
        tipo_autor = row["Tipo de Autor"] if pd.notna(row["Tipo de Autor"]) else None
        porcentaje_autor = (
            row["Porcentaje Reclamado de Autor"]
            if pd.notna(row["Porcentaje Reclamado de Autor"])
            else None
        )

        if nombre_autor and tipo_autor and porcentaje_autor:
            # Insertar o encontrar el autor único
            autor_id = insert_autores_unicos(
                connection, nombre_autor.strip(), numero_ip_autor, tipo_autor.strip()
            )
            # Insertar el porcentaje en la relación ObrasAutores
            insert_obras_autores(
                connection, obra_id_actual, autor_id, float(porcentaje_autor)
            )

        # Procesar códigos MLC y ADREV
        codigo_mlc = row["MLC_CODE"] if pd.notna(row["MLC_CODE"]) else None
        codigo_adrev = row["ADREV_CODE"] if pd.notna(row["ADREV_CODE"]) else None

        # Insertar en subidas_plataforma si al menos uno de los códigos no es nulo
        if codigo_mlc or codigo_adrev:
            insert_subidas_plataforma(
                connection, obra_id_actual, codigo_mlc, codigo_adrev
            )
...
if __name__ == "__main__":
    ...
    if connection:
        ...
        cliente_1 = {
            "nombre": "SAYCO",
            "tipo": "sociedad Gestion colectiva",
            "fecha_recibido": date.today().strftime("%Y-%m-%d"),
        }
        cliente_2 = {
            "nombre": "SAYCE",
            "tipo": "sociedad Gestion colectiva",
        ...
        process_and_load_catalog(
            "CATALOGO_SAYCO_CODIGOS_MLC.xlsx",
            connection,
            cliente_1["nombre"],
            cliente_1["tipo"],
            cliente_1["fecha_recibido"],
        )
        process_and_load_catalog(
            "CATALOGO_SAYCE_MLC_ADREV_CODE.xlsx",
            connection,
            cliente_2["nombre"],
            cliente_2["tipo"],
            cliente_2["fecha_recibido"],
        )
        process_and_load_catalog(
            "catalogo_para_pruebas.xlsx",
            connection,
            cliente_3["nombre"],
            cliente_3["tipo"],
            cliente_3["fecha_recibido"],
        )
