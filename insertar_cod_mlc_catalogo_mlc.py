...
def obtener_claves_autores(row):
    autores = [
        nombre_a_set(f"{row.get(f'WRITER FIRST NAME {i}', '').strip()} {row.get(f'WRITER LAST NAME {i}', '').strip()}")
        for i in range(1, 44) if pd.notna(row.get(f'WRITER FIRST NAME {i}')) and pd.notna(row.get(f'WRITER LAST NAME {i}'))
    ]
    return autores
...
cursor.execute("SELECT o.cod_klaim, o.codigo_iswc, o.titulo, au.nombre_autor FROM obras o JOIN obrasautores oa ON o.cod_klaim = oa.obra_id JOIN autoresunicos au ON oa.autor_id = au.id_autor")
obras_dict = {row["codigo_iswc"]: row for row in cursor.fetchall()}
autores_dict = {tuple(sorted(nombre_a_set(row["nombre_autor"]))): row["cod_klaim"] for row in obras_dict.values()}
...
for index, row in tqdm(df.iterrows(), total=df.shape[0], desc="Progreso", unit="obra"):
    ...
    codigo_mlc = row["MLC SONG CODE"].strip() if pd.notna(row["MLC SONG CODE"]) else None
    ...
    if not obra and clave_unica_autor:
        for autor_set in clave_unica_autor:
            for db_autor_set, obra_id in autores_dict.items():
                if autor_set == db_autor_set:  # 🔹 Comparación exacta sin importar el orden
    ...
    if obra:
        ...
        if codigo_existente:  # Ya tiene código MLC
            sin_cambios.append([titulo, iswc if iswc else clave_unica_autor, codigo_existente])
        else:
            actualizados.append((cod_klaim, codigo_mlc))
            subidas_dict[cod_klaim] = codigo_mlc  # Actualizar cache

    else:
        no_encontrados.append([titulo, iswc if iswc else "N/A", ", ".join([" ".join(a) for a in clave_unica_autor]), codigo_mlc])

# 🚀 Insertar los códigos en una sola consulta optimizada
if actualizados:
    cursor.executemany("INSERT INTO subidas_plataforma (obra_id, codigo_MLC) VALUES (%s, %s)", actualizados)
    conexion.commit()

# 🚀 Convertir DataFrames y Guardar Reporte
df_actualizados = pd.DataFrame(actualizados, columns=["Obra ID", "Código MLC Insertado"])
df_sin_cambios = pd.DataFrame(sin_cambios, columns=["Título", "ISWC o Clave Única", "Código MLC Existente"])
df_no_encontrados = pd.DataFrame(no_encontrados, columns=["Título", "ISWC", "Clave Única Autor", "Código MLC"])
